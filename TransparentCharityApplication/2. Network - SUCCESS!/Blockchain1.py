...
# Constructor
    def __init__(self):
        ...
        self.transactions = []            # transactions is a list to enable multiple transactions... currently only using one transaction per block for demonstration though
        self.create_block(proof = 1, previous_hash = '0', reason='') # create the genesis block
...
class DonateToCharity(Resource):
    def post(self):
        ...
        # checks
        if len(args.sender) < 1 or args.amount < 1:
...
class MoveToBeneficiary(Resource):
    def post(self):
        ...
        # checks
        if len(args.beneficiary) < 1 or args.amount < 1:
            response = "INVALID INPUT!"
